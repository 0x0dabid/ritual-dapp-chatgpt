...
contract PrivateMultiModalChatGPT is Ownable {
    // OpenZeppelin Ownable v5 requires the initial owner to be passed to the base constructor.
    constructor(address initialOwner) Ownable(initialOwner) {
        // If you want the deployer to be the owner, just pass msg.sender in the deploy tx.
        // For convenience, we also allow the zero address which Ownable treats as "no owner" —
        // but at deploy time you should pass your actual EOA address.
    }
    ...
    function _requestMedia(
        ...
        bytes memory input = abi.encode(
            ...
            uint256(100000000),       //12  deliveryMaxPriorityFeePerGas  (0.1 gwei)
    ...
    function _buildOutputConfig(
        ...
        return OutputConfig({
            ...
            maxParam3: p3,
            encryptOutput: false,
            numInferenceSteps: uint16(0),
            guidanceScaleX100: uint16(0),
            seed: uint32(0),
            fps: uint8(0),
            negativePrompt: ""
        });
